...
<?php else: ?>
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <?php foreach ($automacoes as $automacao): ?>
      <?php $rotaSlug = str_replace('/automacoes/', '', $automacao['rota']); ?>
      <div class="bg-surface border border-bord rounded-xl p-5 hover:border-flow/40 transition group">
        <div class="flex items-start justify-between mb-4">
          <div class="w-10 h-10 rounded-lg bg-flow/10 flex items-center justify-center">
            <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="#1FD8C4" stroke-width="1.8"><?= $automacao['icon_svg'] ?: '<circle cx="12" cy="12" r="9"/>' ?></svg>
          </div>
        ...
        <a href="index.php?pagina=<?= Saida::e($rotaSlug) ?>" class="text-xs font-medium text-flow group-hover:gap-2 flex items-center gap-1.5 transition-all">
